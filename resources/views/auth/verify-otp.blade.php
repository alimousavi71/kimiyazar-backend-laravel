<x-layouts.auth :title="__('auth.verify_otp.title')">
    <div class="w-full max-w-md" x-data="otpVerification()" x-init="startCountdown()">
        ...
        <form action="{{ $step === 'registration' ? route('auth.register.verify-otp') : route('auth.password.verify-otp') }}" method="POST" class="space-y-6">
            @csrf
    ...
    <script>
        function otpVerification() {
            return {
                countdown: 120,
                isResending: false,
                otpId: {{ session($step . '.otp_id') ?? 'null' }},
                ...
                async resendOtp() {
                    ...
                    try {
                        const response = await fetch(
                            '{{ $step === "registration" ? route("auth.register.resend-otp") : route("auth.password.resend-otp") }}',
                            {
                                method: 'POST',
                                headers: {
