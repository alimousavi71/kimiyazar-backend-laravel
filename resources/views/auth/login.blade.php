...
<x-layouts.auth :title="$title">
    <div class="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden w-full max-w-md">
        ...
        <div class="bg-gradient-to-br from-blue-600 to-indigo-600 px-6 py-8 text-center">
            ...
            <p class="text-blue-100 text-sm">{{ __('auth.login.subtitle') }}</p>
        </div>
        ...
        <form method="POST" action="{{ route('auth.login') }}" class="p-6 space-y-5">
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center gap-2 cursor-pointer group">
                    ...
                    >
                    <span class="text-sm text-gray-700 group-hover:text-gray-900 transition-colors">{{ __('auth.login.remember_me') }}</span>
                </label>
                <a href="{{ route('auth.password.request') }}" class="text-sm font-medium text-blue-600 hover:text-blue-700 transition-colors">
