...
<a 
    href="{{ $href }}" 
    class="flex items-start gap-3 px-4 py-3 hover:bg-gray-50 transition-colors border-b border-gray-50 {{ $unread ? 'bg-green-50/50' : '' }}"
>
...
<div class="flex-1 min-w-0">
        ...
        @if($unread)
            <span class="shrink-0 w-2 h-2 rounded-full bg-green-600 mt-1.5"></span>
        @endif
        </div>
        @if($description)
