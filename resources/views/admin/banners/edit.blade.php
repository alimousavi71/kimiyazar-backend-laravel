<x-layouts.admin :title="__('admin/banners.forms.edit.title')"
    :header-title="__('admin/banners.forms.edit.header_title')" :breadcrumbs="[
        ...
        ['label' => __('admin/banners.forms.breadcrumbs.edit')]
    ]">
    <div class="mb-4">
        <h2 class="text-lg font-semibold text-gray-900">{{ __('admin/banners.forms.edit.title') }}</h2>
        <p class="text-xs text-gray-600 mt-0.5">{{ __('admin/banners.forms.edit.description') }}</p>
    </div>

    <x-card>
        <x-slot name="title">{{ __('admin/banners.forms.edit.card_title') }}</x-slot>
        ...
        <form id="banner-edit-form" action="{{ route('admin.banners.update', $banner->id) }}" method="POST"
            ...
            <x-form-group :label="__('admin/banners.fields.name')" required :error="$errors->first('name')">
                <x-input type="text" name="name" id="name" :placeholder="__('admin/banners.forms.placeholders.name')"
                    value="{{ old('name', $banner->name) }}" required class="w-full" />
            </x-form-group>
                ...
                <div x-data="{ 
                    ...
                    <x-select name="position" id="position" class="w-full" required
                        ...
                        @foreach($bannerPositions as $position)
                            ...
                            @endphp
                            <option value="{{ $position->value }}" {{ old('position', $banner->position->value) === $position->value ? 'selected' : '' }}
            ...
            <x-form-group :label="__('admin/banners.fields.banner_file')" :error="$errors->first('banner_file')">
                ...
                <x-input type="file" name="banner_file" id="banner_file"
                    accept="image/jpeg,image/png,image/jpg,image/webp" class="w-full" />
                <p class="mt-1 text-xs text-gray-500">{{ __('admin/banners.forms.placeholders.banner_file_update') }}
                </p>
            </x-form-group>
            ...
            <x-form-group :label="__('admin/banners.fields.link')" :error="$errors->first('link')">
                <x-input type="url" name="link" id="link" :placeholder="__('admin/banners.forms.placeholders.link')"
                    value="{{ old('link', $banner->link) }}" class="w-full" />
            </x-form-group>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <x-form-group :label="__('admin/banners.fields.target_type')" :error="$errors->first('target_type')">
                    <x-input type="text" name="target_type" id="target_type"
                        ...
                        value="{{ old('target_type', $banner->target_type) }}" class="w-full" />
                </x-form-group>
                ...
                <x-form-group :label="__('admin/banners.fields.target_id')" :error="$errors->first('target_id')">
                    <x-input type="number" name="target_id" id="target_id"
                        :placeholder="__('admin/banners.forms.placeholders.target_id')"
                        value="{{ old('target_id', $banner->target_id) }}" min="1" class="w-full" />
                </x-form-group>
            </div>

            <x-form-group :label="__('admin/banners.fields.is_active')">
                <x-toggle name="is_active" id="is_active" :checked="old('is_active', $banner->is_active)"
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
                ...
                <x-button variant="primary" type="submit">
                    {{ __('admin/banners.forms.edit.submit') }}
                </x-button>
            </div>
        </form>
    </x-card>
