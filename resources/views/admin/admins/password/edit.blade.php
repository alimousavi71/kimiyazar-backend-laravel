<x-layouts.admin :title="__('admin/admins.forms.password.title')"
    :header-title="__('admin/admins.forms.password.header_title')" :breadcrumbs="[
        ...
        ['label' => __('admin/admins.forms.breadcrumbs.password')]
    ]">
    <div class="mb-4">
        <h2 class="text-lg font-semibold text-gray-900">{{ __('admin/admins.forms.password.title') }}</h2>
        <p class="text-xs text-gray-600 mt-0.5">{{ __('admin/admins.forms.password.description') }}</p>
    </div>

    <x-card>
        <x-slot name="title">{{ __('admin/admins.forms.password.card_title') }}</x-slot>

        <form id="admin-password-form" action="{{ route('admin.admins.password.update', $admin->id) }}" method="POST"
            class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-200">
                ...
                <x-button variant="primary" type="submit">
                    {{ __('admin/admins.forms.password.submit') }}
                </x-button>
            </div>
        </form>
    </x-card>
