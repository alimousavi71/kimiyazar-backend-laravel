<x-layouts.user-profile :title="__('user/orders.show.title')"
    :header-title="__('user/orders.show.header_title')" :breadcrumbs="[
    ...
    <div class="space-y-6">
        ...
        <x-card>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.customer_type') }}</label>
        ...
        <x-card>
            ...
            <div class="space-y-4">
                @if ($order->customer_type === 'real')
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.full_name') }}</label>
                            <p class="text-base text-gray-900 mt-1">{{ $order->full_name ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.national_code') }}</label>
                ...
                @else
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.company_name') }}</label>
                            <p class="text-base text-gray-900 mt-1">{{ $order->company_name ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.economic_code') }}</label>
                            <p class="text-base text-gray-900 mt-1">{{ $order->economic_code ?? 'N/A' }}</p>
                        </div>
                        <div>
                            <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.registration_number') }}</label>
        ...
        <x-card>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.postal_code') }}</label>
                        <p class="text-base text-gray-900 mt-1">{{ $order->postal_code ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.receiver_full_name') }}</label>
                        <p class="text-base text-gray-900 mt-1">{{ $order->receiver_full_name ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.delivery_method') }}</label>
                        <p class="text-base text-gray-900 mt-1">
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-base text-gray-900 mt-1">
                            @if($order->product)
                                <a href="{{ route('products.show', $order->product->slug) }}" class="text-blue-600 hover:text-blue-700">
                                    {{ $order->product->name }}
                                </a>
                            @else
                                N/A
                    ...
                    <div>
                        <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.unit_price') }}</label>
                        <p class="text-base text-gray-900 mt-1">
                            @if($order->unit_price)
                ...
                @if ($order->product_description)
                    <div>
                        <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.product_description') }}</label>
        ...
        <x-card>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.payment_type') }}</label>
                        <p class="text-base text-gray-900 mt-1">
                            {{ __('user/orders.payment_types.' . $order->payment_type->value) }}
                        </p>
                    </div>
                    <div>
                        <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.total_payment_amount') }}</label>
                    ...
                    @if($order->payment_date)
                        <div>
                            <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.payment_date') }}</label>
                            <p class="text-base text-gray-900 mt-1">{{ $order->payment_date }}</p>
                        </div>
                    @endif
                    @if($order->payment_time)
                        <div>
                            <label class="text-sm font-medium text-gray-500">{{ __('user/orders.fields.payment_time') }}</label>
